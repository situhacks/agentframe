...
Available deck/presentation paths for AgentFrame. Load this when a deliverable needs a `.pptx` (or deck-shaped) output and a path hasn't been picked. Usage rules, lock criteria, and export records are owned by the deliverable that calls this menu — brief templates own their own export mechanics; project posts record deck media like any other delivered asset.
