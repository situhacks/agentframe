...
- [ ] **Remove onboarding reminder, delete this checklist, then swap to CMO.** Do in this order:
  1. In [`AGENTS.builder.md`](AGENTS.builder.md), delete the entire **First run** section (the `## First run` heading and the single line that links to this file). Apply the same removal to [`AGENTS.md`](AGENTS.md) if it still matches Builder mode so the two files stay aligned before you swap. (If you prefer to swap first, `AGENTS.md` is overwritten by CMO on the next step — you must still patch `AGENTS.builder.md` so the next Builder swap is clean.)
  2. Delete `onboarding-checklist.md`.
  3. Swap to CMO with `Copy-Item AGENTS.cmo.md AGENTS.md -Force` and append a `mode_swap` row via [`system/audit/writer.py`](system/audit/writer.py). CMO takes it from here for the operator's first campaign.

**Upstream / template repos:** keep committing `onboarding-checklist.md` and the **First run** line in the Builder `AGENTS` files on the default branch so every new clone gets them. Operators who finish onboarding and want a minimal fork may commit the removals on their branch only.
