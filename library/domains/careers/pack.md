...
| Deliverable templates | [`deliverables/`](deliverables/) | `jd-map`, `resume`, `cover-letter`, `company-brief` |
| Verb applicability + hooks | `verbs` above + `rules.py` | no `publish`; `lock` on resume/cover-letter enforces the verification gate + parse-hazard lint (`on_lock`) |
| Doctor rules | `rules.py` `check_application()` | resume-lint (parse hazards), verification freshness |
