...
Offer the options and let the operator narrow; record a campaign-wide preference in `campaign.md` `post_manifest` notes when decks recur in a campaign. Mixing paths is normal — generate with PPT Master or OD, then edit/fill the result with the PPTX skill.
