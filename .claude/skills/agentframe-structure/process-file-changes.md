...
- `library/process/flow-authoring.md` if the proposed process is really a campaign flow

## Procedure

1. **Classify the procedure:** shared process primitive, flow phase rule, deliverable-specific runbook, or always-loaded invariant.
2. **Name the lowest owner:** shared procedures live in `library/process/`; phase sequence lives in `flows/`; deliverable-specific gates live in that deliverable's template; cross-cutting route rules live in `AGENTS*.md`.
3. **Locate before inventing:** extend an existing process file when it already owns the job.
4. **Write for runtime use:** purpose, when to load, procedure, verification/logging, and boundaries. Size budget: 1,500 words (`af doctor` flags overages) — over budget, cut before adding.
