...
1. **Scaffold from the schema.** Generate `library/context/operator/voice/` from the voice schema (`library/context/operator-schema/voice/`): create `identity.md`, `anti-patterns.md`, `voice-profile.md`, `pairs/`, `templates/`, and `intake/corpus/` following the schema's shape. Generate from the shape — there is no example to copy.
