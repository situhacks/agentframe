...
function automationState(row) {
  const issue = row.issues?.[0];
  const text = issue || row.runtime_state || 'not-deployed';
  return el('span', {
    class: `state ${issue ? 'blocked' : row.runtime_state || 'unknown'}`,
    text: `[${plainLabel(text)}]`,
    title: (row.issues || []).join(', ') || row.deployment_id || '',
  });
}

function todayCell(today = {}) {
  const parts = [];
  if (today.done) parts.push(`${today.done} done`);
  if (today.blocked) parts.push(`${today.blocked} blocked`);
  if (today.failed) parts.push(`${today.failed} failed`);
  return el('span', { class: 'mono nowrap', text: parts.join(' / ') || '--' });
}

function automationWorkCell(row) {
  const last = row.last_result;
  const text = row.current_task
    ? `running ${row.current_task}`
    : last ? `${plainLabel(last.status)}: ${last.summary || last.task_id || ''}` : '--';
  const attrs = {
    class: `${row.file ? 'file-link ' : ''}automation-work`,
    text,
    title: last ? [last.task_id, last.time, last.summary].filter(Boolean).join(' · ') : '',
  };
  if (row.file) attrs.onclick = () => navigate('preview', { project: row.project, file: row.file });
  return el('div', {}, el('span', attrs), row.file ? previewAction(row.project, row.file) : null);
}
...
export function renderAutomations(model) {
  ...
  table.append(el('thead', {}, el('tr', {},
    ...
    el('th', { text: 'Desired' }), el('th', { text: 'Observed' }),
    el('th', { text: 'Queue' }), el('th', { text: 'Today' }),
    el('th', { text: 'Current / last result' }))));
  const tbody = el('tbody');
  if (!rows.length) {
    tbody.append(el('tr', { class: 'empty-row' }, el('td', { colspan: '7', text: 'no managed automations declared or deployed' })));
  }
  for (const row of rows) {
    tbody.append(el('tr', {},
      el('td', { class: 'nowrap', title: row.job || '' },
        el('span', { text: plainLabel(row.automation_id || row.deployment_id) })),
      el('td', { class: 'nowrap' }, row.project ? projCell(row.project, row.project_name) : el('span', { text: '--' })),
      el('td', {}, el('span', { class: `state ${row.desired_status || 'unknown'}`, text: `[${plainLabel(row.desired_status)}]` })),
      el('td', {}, automationState(row)),
      el('td', { class: 'mono', text: String(row.queued || 0) }),
      el('td', {}, todayCell(row.today)),
      el('td', {}, automationWorkCell(row))));
  }
  table.append(tbody);
  body.replaceChildren(table);
  const issueCount = rows.filter((row) => row.issues?.length).length;
  document.getElementById('automations-count').textContent = `${rows.length} total${issueCount ? ` · ${issueCount} attention` : ''}`;
}
