...
| 10 Submit | **The human submits** in a normal browser, on the company career site (never a bot; never Easy Apply when direct apply exists) → `af pipe stage <slug> applied`. **Agency or referral submits instead:** the material still ships, to the recruiter, so `af ready` gates it before it leaves the machine exactly as above; then go straight to `af pipe stage <slug> interviewing`, which stamps `shipped` from the ready head and nudges when it is not, and record `submitted_by:` in `application.md` (who submitted, not whether it was verified) | operator |
| 11 Interview | Per round, run the arc in [`interview-arc.md`](interview-arc.md) | process + templates |
| 12 Offer | `af pipe stage <slug> offer --offered-role "<title>"` when the title differs from the posting; `comp-case.md` at the application root per its template — assembled from each round's confirmed-evidence section, worked before the first comp call, and the ask itself goes out through `correspondence/`. Run [`offer-terms.md`](offer-terms.md) alongside it: the comp case argues the number, that file covers the rest of the package and names what to request before answering. The answer is `af pipe stage <slug> accepted` or `declined` | spine + template + process |
| 13 Track | `af doctor` surfaces nudges (7-day silence) and stale rows | spine |
| 14 Close | Terminal stage (`accepted`, `declined`, `rejected`, `ghosted`, `dropped`) → `af pipe archive <slug>` moves the folder to `applications/completed/`. Run [`career-harvest`](../../process/career-harvest.md) first, while the detail is fresh | spine + process |
| 15 Close search | When an acceptance ends the campaign: `af pipe close-search --reason "..."` drops every live row with that reason and stamps the board `search_status: closed`; then update the status line in `search-profile.md` | spine + operator |
