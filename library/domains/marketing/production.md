...
| Publish coordination | the [`post-final`](deliverables/post-final/template.md) template, the post's `post-FINAL.md` named by the tracker, `activity.md` | the target channel profile; [`substack-publishing`](../../process/substack-publishing.md) when the target is Substack; at publish, [`voice-harvest`](../../../system/skills/voice-harvest/SKILL.md) and [`deliverable-harvest`](../../../system/skills/deliverable-harvest/SKILL.md) unconditionally on a user-voiced post (corpus promotion is eligible once the final is live; [`voice-mini-retro`](../../process/voice-mini-retro.md) is the gate) |
...
When the operator provides a published post link, follow the Publish / Export Mechanics procedure in [`deliverables/post-final/template.md`](deliverables/post-final/template.md). State transitions are button-owned: `python system/af.py publish` does the mechanics (published-state record on `post-FINAL.md`, tracker, derived published-post receipt, project `shipped_at`) and prints the judgment checklist.
