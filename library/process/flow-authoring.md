...
4. Deliverable templates load only when drafting, revising, marking ready, publishing, or analyzing that deliverable.
...
3. `Deliverables By Phase` — deliverable names, file targets, and template links. Do not restate deliverable output shapes, hard constraints, readiness criteria, voice/context rules, or export mechanics.
...
6. `Completion Criteria` — project-level terminal gates only. This is not deliverable readiness criteria. Say what makes this flow complete, cancelled, or ready to archive, then point to the templates/process files that own the details.
...
- Do not duplicate deliverable template details. Templates own artifact shape, readiness gates, publish/export details, and per-deliverable frontmatter.
- Do not duplicate shared process primitives. Process files own reusable procedures such as ready events, performance capture, Composio notes, frontmatter schema, humanizer, voice mini-retro, and cancellation routines.
...
- Flow `Deliverables By Phase` must not copy template `Output Shape`, `Hard Constraints`, `Draft Frontmatter Convention`, or `Readiness Criteria`.
...
- Flow `Completion Criteria` covers project-level terminal conditions; deliverable templates own per-deliverable readiness criteria.
