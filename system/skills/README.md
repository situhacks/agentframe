...
| [`agent-reach/`](agent-reach/) | Read-only routing over installed CLIs for sources the default toolchain cannot reach (Reddit, LinkedIn, X, Instagram, Facebook) plus higher-fidelity web, YouTube, semantic-search, GitHub, and RSS paths | WebFetch/WebSearch is refused or returns nothing for a source the task requires, or a task names one of those platforms as a required source | Vendored thin overlay (see [`agent-reach/VENDOR.md`](agent-reach/VENDOR.md)) |
| [`diagram-design/`](diagram-design/) | Editorial diagrams across 39 layout grammars as self-contained HTML/SVG, brand-tokenized per client profile | A structural visual is the requested artifact and a human audience sees it; routed by [`library/process/diagram-production.md`](../../library/process/diagram-production.md) | Vendored (see [`diagram-design/VENDOR.md`](diagram-design/VENDOR.md)) |
| [`d2-diagrams/`](d2-diagrams/) | Auto-laid-out D2 graphs | A graph is regenerated from changing source and layout must re-solve itself; not the default diagram route | Owned by AgentFrame; uses the vendored D2 CLI ([`system/tools/d2/VENDOR.md`](../tools/d2/VENDOR.md)) |
